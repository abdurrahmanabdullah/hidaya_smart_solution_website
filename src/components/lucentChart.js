...
const LucentChartComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:1339/api/lucent-chart');
  ...
  return (
    <div style={{padding:"40px"}}>
      ...
      <img 
        src={`http://localhost:1339${lucentChartData.attributes.image.data.attributes.url}`}
  ...
  <div style={{ marginTop: '90px', textAlign: 'center' }}>
    ...
    <ReactPlayer
      url={`http://localhost:1339${video.data.attributes.url}`}
      width="560px"
      height="315px"
      controls
      style={{ margin: '0 auto' }}
    ...
    {logo.data.map((logoItem, index) => (
      <img
        key={logoItem.id}
        src={`
http://localhost:1339${logoItem.attributes.url}`}
